...
- **Enforced, not just requested.** A prompt instruction to "write a report" is one the model
  can silently skip. A single `SubagentStop` hook (`hooks/ensure-report.sh`) blocks *any*
  agent from finishing until its report under `reports/` actually exists — `write-test.md`,
  `crash_triage.md`, `code-reviewer.md`, or `release-app.md`. One data-driven mechanism turns
  a hoped-for step into a guarantee for the whole kit. Fails open if `jq` is missing so it
  never breaks a run.
